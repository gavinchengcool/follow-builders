...
// Tracks which tweet IDs we've already included in feeds
...
async function main() {
  ...
  if (podcastsOnly) {
    console.error('Podcast support has been removed. Use --tweets-only or --blogs-only, or run without flags for both.');
    process.exit(1);
  }

  // If a specific --*-only flag is set, only that feed type runs.
  // If no flag is set, both supported feed types run.
